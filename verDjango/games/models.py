...
class GameOthello:
    ...
    def play_ai_greedy(self):
        if not self.end:
            z = aig.pick(self.board.b, self.board.wn, self.board.bn, self.board.turn, self.v)
    ...
    def play_ai_minimax(self):
        if not self.end:
            z = aim.pick(self.board.b, self.board.wn, self.board.bn, self.board.turn, self.v, self.depth)
            self.board.flip(z, self.v[z])
            self.rou += 1
            self.prepare_next_round()
